...
let epicReward = require('./reward.js');
	...
	async.parallel({
		...
		config: function (callback) {
			...
			callback(null, {
				...
				// Epic specific
				workerSeparator: '+',
				epicboxDefaultDomain: 'epicbox.epiccash.com',
				algorithms: ['randomx', 'progpow', 'cuckoo'],
				blockMaturity: 1440,
 ...
 * pool.js). Epic has several PoW families with unrelated difficulty units, so the
 * headline `difficulty` is the one of the algorithm the next block must use.
 **/
function getNetworkData (callback) {
	apiInterfaces.nodeApi('/v1/chain', function (error, chain) {
		if (error || !chain || typeof chain.height !== 'number') {
			log('error', logSystem, 'Error getting network data %j', [error ? String(error) : 'bad reply']);
			callback(true);
			return;
		}
...
/**
 * Get Last Block data (chain tip). Mimblewimble hides amounts, so the reward shown here is the one the consensus
 * rules give to miners at this height (no fees); the pool reads the exact reward of its own blocks from its wallet.
 **/
function getLastBlockData (callback) {
	apiInterfaces.nodeApi('/v1/chain', function (error, chain) {
		if (error || !chain || typeof chain.height !== 'number') {
			log('error', logSystem, 'Error getting last block data %j', [error ? String(error) : 'bad reply']);
			callback(true);
			return;
		}

		apiInterfaces.nodeApi('/v1/headers/' + chain.height, function (error, header) {
			if (error || !header || !header.hash) {
				log('error', logSystem, 'Error getting last block header %j', [error ? String(error) : 'bad reply']);
				callback(true);
				return;
			}
			callback(null, {
				...
				timestamp: Math.floor(Date.parse(header.timestamp) / 1000),
				reward: epicReward.minerReward(header.height),
				hash: header.hash
			});
		});
	});
 ...
 **/
/**
 * Shorten an epicbox address for the public top 10: the ends of the key, not of the whole string (every address ends with the
 * same "@epicbox.epiccash.com"). The default domain is left out, another domain is kept so that addresses stay distinguishable.
 **/
function shortMinerAddress (account) {
	// a deposit note identifies a customer of an exchange: show only its last 3 characters
	let split = utils.splitMinerAccount(account);
	let address = split.address;
	let noteMask = split.note ? '#...' + split.note.slice(-3) : '';
	let at = address.indexOf('@');
	let key = at === -1 ? address : address.substring(0, at);
	let domain = at === -1 ? '' : address.substring(at + 1);
	let shortKey = key.length > 16 ? key.substring(0, 8) + '...' + key.substring(key.length - 6) : key;
	return (domain && domain !== 'epicbox.epiccash.com' ? shortKey + '@' + domain : shortKey) + noteMask;
}
